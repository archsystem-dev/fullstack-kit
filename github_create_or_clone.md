...
## Objectif du script
Crée un dépôt privé GitHub si inexistant ou clone le dépôt existant, initialise Git localement et effectue le premier push avec les identifiants configurés dans github.ini.
...
4. Extrait le nom du projet et vérifie la présence de `github.ini`.
5. Charge les paramètres GitHub (user, token, name, email) depuis `github.ini`.
...
- Fichier `github.ini` présent dans le répertoire du projet avec une section `[Github]` contenant les clés `user`, `token`, `name` et `email`.
